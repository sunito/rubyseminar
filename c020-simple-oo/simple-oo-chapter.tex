...
puts adr.zeile
  ...
  \pause
    ...
    \path[->]
                  node[klasse] (Adresse) {\klassebox{Adresse}{zeile \\ plz \\ ort}};
    \pause
    \path[->]
                  node[objekt, below left of=Adresse] (adr) {\klassebox{adr}{@plz = 17268 \\ @ort = Templin}}
    ...
    \path[->]     (adr.east) node[anchor=west,font=\footnotesize] (meth-adr) {.zeile} (adr.east)
                  (meth-adr) edge[method1,controls=+(0:2) and +(180:2)] node[sloped, near end]{} (Adresse.west);
\end{tikzpicture}
