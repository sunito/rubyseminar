...
\begin{frame}[fragile]
  ...
  \begin{tikzpicture}[node distance=3cm, auto,>=latex', thick]
    ...
    \path[->]<1-> node[klasse, below of=Person, label={[name=lNatPerson]left:}] (NatPerson) {NatPerson}
                  (NatPerson) edge[superc] (Person);
    \path[<->]<1-> 
                  node[objekt, below left of=NatPerson] (heinz) {heinz}
                  (heinz) edge[class] node {class} (NatPerson)
                  (heinz) edge[methods,<=*] (lNatPerson.west)
                  (lNatPerson.west) edge[methods] (Person.west);
\end{tikzpicture}
\end{frame}
